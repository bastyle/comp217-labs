...
{
public:
    void towerOfHanoi(int n, char source, char auxiliary, char destination)const {
        ...
        while (n >= 1 or !st.empty()) {
            ...
            Toh cur = st.top();
            st.pop();
            cout << "Move disk " << cur.getDiskNumber() << " from rod " << cur.getOriginalPeg() <<
                " to rod " << cur.getDestinationPeg() << std::endl;
            if (cur.getDiskNumber() >= 1) {
                source = cur.getTemporalPeg();           // Simulates the towerOfHanoi(n-1,auxiliary,source,destination) of Recursive Step.
